...
public class ImageCroppingProcess implements WorkflowProcess {
	private final Logger LOGGER = LoggerFactory.getLogger(ImageCroppingProcess.class);
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap metaDataMap) {
		...
		try {
			resourceResolver = resourceResolverService.getResourceResolver(Constant.SUB_SERVICE);
		} catch (LoginException e) {
			LOGGER.error("Error creating resource resolver using sub-service {} , {}", Constant.SUB_SERVICE, e);
		}
				...
				if (node != null) {
					...
					try {
						if (!EMPTY.equals(processArgument)) {
							List<String> args = EnablementUtils.splitWithNewLine(processArgument);
							for (String arg : args) {
								Layer originalLayer = ImageHelper.createLayer(dataResource);
								ImageHelper.saveLayer(getLayer(originalLayer, payload, arg), JPEG_MIME_TYPE, quality,
										node, CQ5_DAM_THUMBNAIL + arg + JPEG_EXTENSION, true);
							}
						}
					} catch (Exception exception) {
						LOGGER.error(exception.getCause().toString());
					} finally {
						EnablementUtils.closeResourceResolver(resourceResolver);
					}
				}
			}
		}
